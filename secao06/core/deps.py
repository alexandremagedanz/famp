...
from core.databese import Session, engine
...
async def get_section() -> AsyncGenerator:
...
async def get_current_user(token: str = Depends(oauth2_schema), db: AsyncSession  = Depends(get_section)) -> UsuarioModel:
    ...
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == int(str(token_data.username)))
